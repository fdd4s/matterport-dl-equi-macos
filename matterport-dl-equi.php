...
if (is_executable(dirname(__FILE__)."/sky2equi.sh")==false) exec("chmod +x ".dirname(__FILE__)."/sky2equi.sh");
if (is_executable(dirname(__FILE__)."/mattersky2equi.sh")==false) exec("chmod +x ".dirname(__FILE__)."/mattersky2equi.sh");
if (is_executable(dirname(__FILE__)."/resize_equi.sh")==false) exec("chmod +x ".dirname(__FILE__)."/resize_equi.sh");
...
mkdir(dirname(__FILE__)."/matterport-".$id."-equi/");
if ($sky_size==4) mkdir(dirname(__FILE__)."/matterport-".$id."-equi-m/");

$num = 1;
foreach($lista as $item) {
	$name = getName($item);
	if(file_exists($name."equi.jpg")==false) {
		system(dirname(__FILE__)."/mattersky2equi.sh \"".$item."\"");
	}
	if($sky_size==4 && (file_exists($name."equi-m.jpg")==false)) {
		system(dirname(__FILE__)."/resize_equi.sh \"".$name."equi.jpg\" \"".dirname(__FILE__)."/matterport-".$id."-equi-m/matterport-".$id."-equi-m_".$num.".jpg\" 8192");
		echo("matterport-".$id."-equi-m_".$num.".jpg created\n");
	}
	if (file_exists($name."equi.jpg")) {
		cleanSkybox($name);
		rename($name."equi.jpg", dirname(__FILE__)."/matterport-".$id."-equi/matterport-".$id."-equi_".$num.".jpg");
		echo("matterport-".$id."-equi_".$num.".jpg created\n");
	}
	$num++;
}
...
function cleanSkybox($name) {
	for ($i=0;$i<6;$i++) unlink($name."skybox".$i.".jpg");
}

function getName($skyName) {
